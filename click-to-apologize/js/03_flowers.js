...
function Flower([x, y]) {
    ...
    this.scale = 0;
    this.maxScale = 2 * Math.pow(Math.random(), 30);
...
function recreateInstancedMesh() {
    ...
    particles.forEach(p => {
        if (p.type === 0) {
            flowerInstancedMesh.setColorAt(flowerIdx, new THREE.Color("hsl(0, 100%, 50%)"));
            flowerIdx ++;
        } else {
            leafInstancedMesh.setColorAt(leafIdx, new THREE.Color("hsl( 0, 100%, 50%)"));
            leafIdx ++;
        }
    })
